...
```{r}
ggplot(iris_plot, aes(species, sepal_width)) +
...
2. Let's compute the mean Agreeableness score across items for each participant by using rowwi
...
psych::bfi %>%
  ...
  mutate(p_min = pmin(C1, C2, C3, C4, C5))
```
...
m_bfi <-
  ...
  mutate(A_mean = mean(c(A1, A2, A3, A4, A5)),
         C_mean = mean(c(C1, C2, C3, C4, C5)),
         E_mean = mean(c(E1, E2, E3, E4, E5)),
         N_mean = mean(c(N1, N2, N3, N4, N5)),
         O_mean = mean(c(O1, O2, O3, O4, O5))) %>%
